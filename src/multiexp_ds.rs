    ...
    S: SourceBuilder<G>,
{
    ...
    parts
        ...
        .try_fold(<G as CurveAffine>::Projective::zero(), |mut acc, part| {
            // let mut c_cnt = 0; // debug
            for _ in 0..c {
                // c_cnt += 1;
                acc.double();
            }
            // println!("[DEBUG-multiexp.rs-target-count] c_cnt:{:?}", c_cnt); 
            acc.add_assign(&part?);
            Ok(acc)
        })
}
...
// cargo test multiexp_ds::me_test --features=pairing -- --nocapture
#[cfg(feature = "pairing")]
#[test]
fn me_test(){
...
// cargo test multiexp_ds::test_with_bls12 --features=pairing -- --nocapture
